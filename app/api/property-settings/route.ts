...
export async function POST(request: NextRequest) {
  try {
    ...
    // Update property name in properties table if it was provided
    if (validatedData.property_name && validatedData.property_name !== property.name) {
      const { error: updatePropertyError } = await supabase
        .from('properties')
        .update({ name: validatedData.property_name })
        .eq('id', validatedData.property_id);

      if (updatePropertyError) {
        console.error('Error updating property name:', updatePropertyError);
        return NextResponse.json({ error: 'Failed to update property name' }, { status: 500 });
      }
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (propertyError || !property) {
      return NextResponse.json({ error: 'No property found for this user' }, { status: 404 });
    }
