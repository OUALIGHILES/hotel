...
CREATE TABLE IF NOT EXISTS external_accounts (
  ...
  platform TEXT NOT NULL, -- e.g., 'airbnb', 'booking_com', 'vrbo'
  external_account_id TEXT, -- The platform's unique identifier for the account
  access_token TEXT,
  refresh_token TEXT,
...
CREATE TABLE IF NOT EXISTS listing_sync (
  ...
  external_listing_id TEXT NOT NULL, -- External platform's listing ID
...
CREATE POLICY "listing_sync_select_own" ON public.listing_sync
  FOR SELECT USING (
    ...
    (
      ...
      WHERE id = (SELECT property_id FROM units WHERE id = listing_sync.pms_unit_id)
    ) = auth.uid()
  );

CREATE POLICY "listing_sync_insert_own" ON public.listing_sync
  FOR INSERT WITH CHECK (
    (SELECT user_id FROM external_accounts WHERE id = external_account_id) = auth.uid()
    AND
    (SELECT user_id FROM properties WHERE id = (SELECT property_id FROM units WHERE id = pms_unit_id)) = auth.uid()
  );
...
CREATE POLICY "listing_sync_update_own" ON public.listing_sync
  FOR UPDATE USING (
    ...
    AND
    (SELECT user_id FROM properties WHERE id = (SELECT property_id FROM units WHERE id = listing_sync.pms_unit_id)) = auth.uid()
  );
